...
def _overal_watershed_marker(probmap,marker,  thresh_seg = 0.7,ratio = 0.2, compactness = 3):

    #npad2 = ((1,1),(1,1))
    #probmap = np.pad(probmap, npad2, mode='constant', constant_values=0)
    #marker =  np.pad(marker, npad2, mode='constant', constant_values=0)

    label = morphology.watershed(probmap, marker)
    return label

def overal_watershed_marker(probmap,marker,  thresh_seg = 0.7,ratio = 0.2, dist_f = np.median,**kwargs):
    ...
    #label = cv2.watershed(fg_mask_color, markers)

    #fg_mask_color = cv2.cvtColor(fg_mask.astype('uint8'),cv2.COLOR_GRAY2RGB)
    #markers = cv2.watershed(fg_mask_color, markers)

    markers[fg_mask==0] = -1
    label = random_walker(fg_mask, markers)

    label[fg_mask == 0] = 0
    remove_boarder_label = label[1:-1, 1:-1]
    return remove_boarder_label
...
def overal_watershed_(probmap, thresh_water = 0.6, thresh_seg = 0.5,ratio = 0.2, dist_f = np.median, **kwargs):
    npad2 = ((1,1),(1,1))
    probmap = np.pad(probmap, npad2, mode='constant', constant_values=0)
    ...
    markers[~fg_mask] = -1
    markers = random_walker(fg_mask, markers)
...
def overal_watershed(probmap, thresh_water = 0.8, thresh_seg = 0.2, ratio = 0.2, dist_f = np.median, org_img=None,
                     tiny_size = 100, thresh_remove = 190, thre_ratio = 1.2):
    ...
    region_count = 1
    first_regions = regionprops(first_pass)[1:] #because cv2 label image start with 1
    second_regions = regionprops(second_pass)[1:]

    for props in first_regions :
        this_mean = np.mean(gray_img[first_pass ==  props.label]) if gray_img is not None else -np.inf
        #print this_mean
...
def single_watershed(probmap, thresh_water = 0.8, thresh_seg = 0.2,ratio = 0.3, dist_f = np.max,org_img=None,
    ...
    region_count = 1
    for props in regions[1:]: #necause cv2 label start with 1,
        this_mean = np.mean(gray_img[label_img ==  props.label]) if gray_img is not None else -np.inf
        #print this_mean
        if props.area < tiny_size or touchboundary(props.bbox, probmap.shape[0:2]) or (this_mean > thre_ratio*mean_value or this_mean  > thresh_remove):
            ...
            for tmp_props in regionprops(temp_seg)[1:]:
                this_mean = np.mean(gray_img[label_img ==  tmp_props.label]) if gray_img is not None else -np.inf
